...
for every_file in li:#(os.listdir(data_path))[346:]:
    ...
    wrtr = csv.writer(open(cwd + "/wordList/" + every_file[:-4] + ".csv", "w"))
    ...
    for s in doc:
        ...
        while w < len(words) - 2:
            ...
            for b in bi:
                # print('andar', w, len(words))
                if last == b[0][0] and first == b[0][1]:
                    continue
                if words[w] == b[0][0] and words[w + 1] == b[0][1]:

                    f = 0
                    for t in tri:
                        if last == t[0][1] and first == t[0][2]:
                            continue
